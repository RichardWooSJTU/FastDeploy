...
class KVCacheBenchmark:
    def __init__(self, splitwise_role: str, config: BenchmarkConfig, port: int = None, device: str = "gpu"):
        ...
        print(f"[Benchmark] Config: layers={config.num_layers}, max_blocks={config.max_block_num}, "
        ...
        print(f"[Benchmark] Total cache memory: {self._format_bytes(self.bytes_per_block * config.max_block_num * config.num_layers)}")
    ...
    def write_cache(self, ip: str, port: int, query_block_ids_list: List[List[int]]):
        ...
        """
        assert self.splitwise_role == "prefill", "only prefill can call this method"
        assert len(query_block_ids_list) == self.config.num_queries, \
    ...
    def run_benchmark(self, ip: str, port: int) -> BenchmarkResult:
        ...
        blocks_per_query = min(self.config.blocks_per_transfer,
                               self.config.max_block_num // self.config.num_queries)
...
def parse_args():
    ...
    parser.add_argument("--splitwise_role", type=str, default="prefill",
                        choices=["prefill", "decode"], help="Role: prefill (client) or decode (server)")
    ...
    parser.add_argument("--cache_dtype", type=str, default="bfloat16",
                        choices=["bfloat16", "float16", "uint8"], help="Cache data type")
    ...
    parser.add_argument("--blocks_per_transfer", type=int, default=64,
                        help="Number of blocks per transfer")
    parser.add_argument("--num_queries", type=int, default=4,
                        help="Number of queries per layer to simulate")
...
def run_decode_server(args, config: BenchmarkConfig):
    ...
    benchmark = KVCacheBenchmark("decode", config, port=args.decode_rdma_port, device=args.device)
