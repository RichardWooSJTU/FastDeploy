...
def depermute_prefill_combine_kernel(
    ...
    ori_2_permute_indice_tensor = smem.allocate_tensor(indice_map.element_type, cute.make_ordered_layout((TOP_K), order=(0)), 16)
    topk_weights_tensor = smem.allocate_tensor(topk_weights.element_type, cute.make_ordered_layout((TOP_K), order=(0)), 16)
    ...
    for token_idx in range(bidx, num_worst_tokens, grid_dimx):
        ...
        for i in cutlass.range_constexpr(cute.size(frag_depermuted_x_coord)):
        ...
        for expert_slot in range(TOP_K):
            ...
            if indice >= 0:
                ...
                permuted_local_expert = indice // max_num_tokens_per_expert
                permuted_offset_in_expert = indice % max_num_tokens_per_expert
                ...
                for i in cutlass.range_constexpr(cute.size(frag_x_coord)):
...
def depermute_prefill_combine(
    ...
    depermute_prefill_combine_kernel(x, x_coord, indice_map, topk_weights, 
                                          depermuted_x,  depermuted_x_coord,
