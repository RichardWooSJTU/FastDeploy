...
def prefill_permute_to_masked_gemm_kernel(
    ...
    offset_tensor = smem.allocate_tensor(token_nums_per_expert.element_type, cute.make_ordered_layout(1, order=(0)), 16)
        ...
        for expert_slot in range(TOP_K):
            ...
            if expert_idx != -1:
                if tidx == 0:
                    ...
                    permuted_indice_map[token_idx, expert_slot] = expert_idx * max_num_tokens_per_expert + offset_tensor[0]
                    # cute.printf("expert_idx {}, offset_tensor {}", expert_idx, offset_tensor[0])
                ...
                thread_scale_coord = thread_copy_x.partition_S(local_scale_coord)
                ...
                thread_scale = thread_copy_x.partition_S(local_scale)
                thread_permute_scale = thread_copy_x.partition_D(local_permute_scale)
...
def prefill_permute_to_masked_gemm(
    ...
    prefill_permute_to_masked_gemm_kernel(x, x_coord, scale, scale_coord, topk_ids, permute_x, 
                                          permute_scale, permuted_indice_map, token_nums_per_expert, 
                                          thr_layout, val_layout, num_tokens, 
