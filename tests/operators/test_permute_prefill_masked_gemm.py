"""
# Copyright (c) 2026  PaddlePaddle Authors. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License"
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
"""

import unittest

import numpy as np
import paddle

from fastdeploy.model_executor.ops.cute_dsl_ops.permute_prefill_masked_gemm import (
    call_prefill_permute_to_masked_gemm,
)


class TestPrefillPermuteToMaskedGemm(unittest.TestCase):
    """
    Test cases for prefill_permute_to_masked_gemm kernel.

    The kernel performs the following:
    - Input:
        - recv_x: [num_worst_tokens, hidden_size] - hidden states from deep ep
        - recv_scale: [num_worst_tokens, hidden_scale] - scale for quantization
        - recv_topk_idx: [num_worst_tokens, topk] - expert indices for each token
          values in range [-1, num_local_experts), -1 means not routed to current rank
    - Output:
        - permute_x: [num_local_experts, num_max_tokens, hidden_size]
        - permute_scale: [num_local_experts, num_max_tokens, hidden_scale]
        - permuted_indice_map: [num_worst_tokens] - maps token_idx to permuted position
          (expert_idx * max_token_num + offset)
        - token_nums_per_expert: [num_local_experts] - count of tokens per expert
    """

    def setUp(self):
        paddle.seed(2024)
        np.random.seed(2024)

    def _get_expected_tokens_per_expert(
        self,
        x: np.ndarray,
        scale: np.ndarray,
        topk_ids: np.ndarray,
        num_local_experts: int,
    ):
        """
        Get the expected set of tokens for each expert.

        Returns:
            expert_to_tokens: dict mapping expert_idx -> list of (x_row, scale_row) tuples
            token_nums_per_expert: [num_local_experts]
        """
        num_tokens = x.shape[0]
        _, topk = topk_ids.shape

        expert_to_tokens = {i: [] for i in range(num_local_experts)}
        token_nums_per_expert = np.zeros(num_local_experts, dtype=np.int32)

        for token_idx in range(num_tokens):
            for k in range(topk):
                expert_idx = topk_ids[token_idx, k]
                if expert_idx != -1:
                    expert_to_tokens[expert_idx].append((x[token_idx, :].copy(), scale[token_idx, :].copy()))
                    token_nums_per_expert[expert_idx] += 1

        return expert_to_tokens, token_nums_per_expert

    def _run_and_verify(
        self,
        num_tokens: int,
        hidden_size: int,
        hidden_scale: int,
        num_local_experts: int,
        max_token_num: int,
        topk: int,
        x_dtype=paddle.float8_e4m3fn,
        scale_dtype=paddle.float32,
        sparsity: float = 0.3,
    ):
        """
        Run the kernel and verify against reference implementation.

        Args:
            num_tokens: number of input tokens
            hidden_size: hidden dimension
            hidden_scale: scale dimension (typically hidden_size // 128)
            num_local_experts: number of local experts
            max_token_num: max tokens per expert
            topk: number of experts each token routes to
            x_dtype: data type for x
            scale_dtype: data type for scale
            sparsity: probability of expert_idx being -1 (not routed)
        """
        # Generate input data
        if x_dtype == paddle.float8_e4m3fn:
            x_np = np.random.randn(num_tokens, hidden_size).astype(np.float32)
            x_np = np.clip(x_np, -448, 448)  # FP8 range
            x = paddle.to_tensor(x_np).cast(paddle.float8_e4m3fn)
        elif x_dtype == paddle.bfloat16:
            x_np = np.random.randn(num_tokens, hidden_size).astype(np.float32)
            x = paddle.to_tensor(x_np).cast(paddle.bfloat16)
        else:
            x_np = np.random.randn(num_tokens, hidden_size).astype(np.float32)
            x = paddle.to_tensor(x_np)

        scale_np = np.random.rand(num_tokens, hidden_scale).astype(np.float32)
        scale = paddle.to_tensor(scale_np).cast(scale_dtype)

        # Generate topk_ids with some -1 values (not routed to current rank)
        # Each token should have unique expert indices (no duplicates within a row)
        topk_ids_np = np.zeros((num_tokens, topk), dtype=np.int64)
        for i in range(num_tokens):
            # Sample unique experts for this token
            experts = np.random.choice(num_local_experts, size=min(topk, num_local_experts), replace=False)
            if len(experts) < topk:
                # If not enough experts, pad with -1
                topk_ids_np[i, :len(experts)] = experts
                topk_ids_np[i, len(experts):] = -1
            else:
                topk_ids_np[i, :] = experts
        # Apply sparsity - randomly set some values to -1
        mask = np.random.rand(num_tokens, topk) < sparsity
        topk_ids_np[mask] = -1
        topk_ids = paddle.to_tensor(topk_ids_np).cast(paddle.int64)

        # Run the kernel
        # print("x", x.astype('float32').sum(axis=1))
        # print("topk_ids", topk_ids)
        permute_x, permute_scale, permuted_indice_map, token_nums_per_expert = call_prefill_permute_to_masked_gemm(
            x=x,
            scale=scale,
            topk_ids=topk_ids,
            num_local_experts=num_local_experts,
            max_token_num=max_token_num,
        )

        # Get numpy results from kernel
        permute_x_result = permute_x.cast(paddle.float32).numpy()
        permute_scale_result = permute_scale.numpy()
        permuted_indice_map_result = permuted_indice_map.numpy()
        token_nums_result = token_nums_per_expert.numpy().flatten()


        # Compute reference: get expected tokens per expert
        x_ref_np = x.cast(paddle.float32).numpy()
        scale_ref_np = scale.numpy()
        expert_to_tokens, token_nums_ref = self._get_expected_tokens_per_expert(
            x=x_ref_np,
            scale=scale_ref_np,
            topk_ids=topk_ids_np,
            num_local_experts=num_local_experts,
        )

        # Verify token counts
        np.testing.assert_array_equal(
            token_nums_result,
            token_nums_ref,
            err_msg=f"Token counts mismatch: kernel={token_nums_result}, ref={token_nums_ref}"
        )

        # Verify permuted data for each expert
        # Due to atomic operations, the order of tokens may differ, so we verify
        # by comparing sorted row sums (as a fingerprint for each row)
        for expert_idx in range(num_local_experts):
            num_tokens_for_expert = token_nums_ref[expert_idx]
            # print(f"expert_idx {expert_idx} num_tokens_for_expert {num_tokens_for_expert}")
            if num_tokens_for_expert > 0:
                # Get actual output rows from kernel
                kernel_x_rows = permute_x_result[expert_idx, :num_tokens_for_expert, :]
                kernel_scale_rows = permute_scale_result[expert_idx, :num_tokens_for_expert, :]

                # Get expected rows
                expected_tokens = expert_to_tokens[expert_idx]
                expected_x_rows = np.array([t[0] for t in expected_tokens])
                expected_scale_rows = np.array([t[1] for t in expected_tokens])

                # Compare using sorted row sums as fingerprints
                # This is order-independent and catches data corruption
                kernel_x_sums = np.sort(np.sum(kernel_x_rows, axis=1))
                expected_x_sums = np.sort(np.sum(expected_x_rows, axis=1))

                np.testing.assert_allclose(
                    kernel_x_sums,
                    expected_x_sums,
                    rtol=1e-2,
                    atol=1e-1,
                    err_msg=f"Expert {expert_idx}: permute_x row sums mismatch"
                )

                kernel_scale_sums = np.sort(np.sum(kernel_scale_rows, axis=1))
                expected_scale_sums = np.sort(np.sum(expected_scale_rows, axis=1))

                np.testing.assert_allclose(
                    kernel_scale_sums,
                    expected_scale_sums,
                    rtol=1e-2,
                    atol=1e-2,
                    err_msg=f"Expert {expert_idx}: permute_scale row sums mismatch"
                )

        # Verify permuted_indice_map: for each token, check that it points to correct data
        # permuted_indice_map[token_idx] = expert_idx * max_token_num + offset
        x_ref_np = x.cast(paddle.float32).numpy()
        for token_idx in range(num_tokens):
            for expert_slot in range(topk):
                # Get the first valid expert for this token (kernel only records last one due to loop)
                # Actually kernel records for each expert_slot, but overwrites, so we check consistency
                permuted_idx = permuted_indice_map_result[token_idx, expert_slot]
                if permuted_idx >= 0:  # Valid mapping
                    expert_idx = permuted_idx // max_token_num
                    offset = permuted_idx % max_token_num
                    # Verify the data at this position matches the original token
                    permuted_data = permute_x_result[expert_idx, offset, :]
                    original_data = x_ref_np[token_idx, :]
                    np.testing.assert_allclose(
                        permuted_data,
                        original_data,
                        rtol=1e-2,
                        atol=1e-1,
                        err_msg=f"Token {token_idx}: permuted_indice_map points to wrong data"
                    )

        return True

    def test_basic_topk4(self):
        """Test basic case with topk=4"""
        self._run_and_verify(
            num_tokens=64,
            hidden_size=7168,
            hidden_scale=56,  # 7168 // 128
            num_local_experts=8,
            max_token_num=128,
            topk=4,
            sparsity=0.2,
        )

    def test_basic_topk8(self):
        """Test basic case with topk=8"""
        self._run_and_verify(
            num_tokens=64,
            hidden_size=7168,
            hidden_scale=56,
            num_local_experts=8,
            max_token_num=128,
            topk=8,
            sparsity=0.2,
        )

    def test_small_tokens(self):
        """Test with small number of tokens"""
        self._run_and_verify(
            num_tokens=4,
            hidden_size=1024,
            hidden_scale=8,
            num_local_experts=4,
            max_token_num=32,
            topk=4,
            sparsity=0.1,
        )

    def test_large_tokens(self):
        """Test with large number of tokens"""
        self._run_and_verify(
            num_tokens=512,
            hidden_size=4096,
            hidden_scale=32,
            num_local_experts=16,
            max_token_num=256,
            topk=4,
            sparsity=0.3,
        )

    def test_high_sparsity(self):
        """Test with high sparsity (many -1 values in topk_ids)"""
        self._run_and_verify(
            num_tokens=128,
            hidden_size=2048,
            hidden_scale=16,
            num_local_experts=8,
            max_token_num=64,
            topk=4,
            sparsity=0.7,
        )

    def test_no_sparsity(self):
        """Test with no sparsity (all tokens routed)"""
        self._run_and_verify(
            num_tokens=64,
            hidden_size=2048,
            hidden_scale=16,
            num_local_experts=8,
            max_token_num=128,
            topk=4,
            sparsity=0.0,
        )

    def test_single_expert(self):
        """Test with single local expert"""
        self._run_and_verify(
            num_tokens=32,
            hidden_size=1024,
            hidden_scale=8,
            num_local_experts=1,
            max_token_num=64,
            topk=4,
            sparsity=0.0,
        )

    def test_many_experts(self):
        """Test with many local experts"""
        self._run_and_verify(
            num_tokens=128,
            hidden_size=2048,
            hidden_scale=16,
            num_local_experts=32,
            max_token_num=64,
            topk=8,
            sparsity=0.3,
        )

    def test_bfloat16_input(self):
        """Test with bfloat16 input"""
        self._run_and_verify(
            num_tokens=64,
            hidden_size=2048,
            hidden_scale=16,
            num_local_experts=8,
            max_token_num=128,
            topk=4,
            x_dtype=paddle.bfloat16,
            sparsity=0.2,
        )

    def test_all_minus_one(self):
        """Test edge case where all topk_ids are -1 (nothing routed to this rank)"""
        num_tokens = 32
        hidden_size = 1024
        hidden_scale = 8
        num_local_experts = 4
        max_token_num = 64
        topk = 4

        # Create input data
        x_np = np.random.randn(num_tokens, hidden_size).astype(np.float32)
        x_np = np.clip(x_np, -448, 448)
        x = paddle.to_tensor(x_np).cast(paddle.float8_e4m3fn)

        scale_np = np.random.rand(num_tokens, hidden_scale).astype(np.float32)
        scale = paddle.to_tensor(scale_np)

        # All -1 topk_ids
        topk_ids = paddle.full([num_tokens, topk], -1, dtype=paddle.int32)

        # Run the kernel
        permute_x, permute_scale, permuted_indice_map, token_nums_per_expert = call_prefill_permute_to_masked_gemm(
            x=x,
            scale=scale,
            topk_ids=topk_ids,
            num_local_experts=num_local_experts,
            max_token_num=max_token_num,
        )

        # Verify all counts are 0
        token_nums_result = token_nums_per_expert.numpy().flatten()
        expected = np.zeros(num_local_experts, dtype=np.int32)
        np.testing.assert_array_equal(token_nums_result, expected)

        # Verify permuted_indice_map shape
        self.assertEqual(permuted_indice_map.shape, [num_tokens, topk])


if __name__ == "__main__":
    unittest.main()
